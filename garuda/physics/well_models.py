...
class PeacemanWell:
    ...
    def compute_effective_radius(self, dx: float, dy: float, kx: float, ky: float) -> float:
        ...
        if kx > 0 and ky > 0:
            ...
            r_0 = 0.28 * numerator / denominator
        else:
...
class WellManager:
    ...
    def compute_well_rates(
        ...
        for well_name, well in self.wells.items():
            ...
            # Get cell dimensions
            if hasattr(grid, 'dx'):
                dx = grid.dx if np.isscalar(grid.dx) else grid.dx[0]
            else:
                dx = 100.0
            
            if hasattr(grid, 'dy'):
                dy = grid.dy if np.isscalar(grid.dy) else grid.dy[0]
            else:
                dy = 100.0
            
            if hasattr(grid, 'dz'):
                dz = grid.dz if np.isscalar(grid.dz) else grid.dz[0]
            else:
                dz = 10.0
    ...
    def get_well_summary(self) -> dict:
        ...
        for name, well in self.wells.items():
            ...
            if is_producer:
                ...
                summary['injectors'] += 1
                summary['total_injection'] += well.current_rate
